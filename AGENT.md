...
- **Verify PHP Syntax**: Always run `php -l` on any modified or newly created PHP files to verify their syntax before completing a task. Be extremely careful with strings or comments containing literal PHP closing tags (`?` followed by `>`), as they will terminate the PHP interpreter block even inside string literals or comments; split them (e.g., `'?' . '>'`) to prevent parse errors.
